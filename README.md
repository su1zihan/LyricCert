...
- Flip card metrics with raw model outputs
- Real-time analysis with modern UI
- Conference webpage with FastAPI backend (`lyriccert.html`)
...
Download the pre-trained checkpoint from the link below and place it inside a new directory named `model/`:
...
### Conference Webpage (`lyriccert.html`)

The conference webpage requires the FastAPI backend to be running alongside.
...
Both Streamlit and FastAPI can run at the same time — they are independent and do not interfere with each other.
...
|----------|-------------|---------|
| `--model-path` | Path to the model directory | `./model` |
| `--port` | Port number | `8501` |
